...
helm  install $ns  argo/argo-cd $kubeconfig --namespace $ns --set notifications.enabled=false --set dex.enabled=false --set redis.enabled=true --set server.replicas=1 --set configs.cm.admin.enabled=false --set applicationSet.replicas=0 --create-namespace
